...
- **Pressure-scenario testing** — before each release, the skill is tested against 67+ adversarial scenarios spanning nine categories: (1) stop-signal recognition (explicit, Chinese, ambiguous, retracted, mid-task, pause signal, stop-during-analysis-phases, stop-during-inter-cycle-pause, post-stop re-engagement with ambiguous trigger phrase, stop-word in code context), (2) completion pressure (false completion, empty backlog, scaffold-only, summary-and-pause, inter-cycle pause format), (3) social/external pressure (praise, time pressure, resource cost), (4) domain coverage (code, ML/research, academic writing, design, documentation, ML metric regression), (5) mid-task behaviors (user feedback, critical-issue discovery, safety boundary, git conflict, batch-commit, CI failure in reflect, user claims task, FIXME-from-prior-cycle, user redirects mid-cycle, status-request mid-work, rework-on-feedback), (6) planning integrity (solvability gate, credentials gate, meta-planning procrastination, stuck-loop refresh, frontier tasks, evaluator-weakening self-audit, high-leverage over micro-tasks, discovered-not-listed output bar, all-tasks-blocked interrupt, micro-task grouping), (7) context recovery (compaction, no guidance files, greenfield, context-window pressure), (8) multi-signal edge cases (retracted stop, stop-with-praise, stop-in-quotes, stop-during-plan-phase), and (9) meaningful work per cycle (deep-over-shallow, frontier scan with high ambition). All scenarios must pass
