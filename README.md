...
Evals are in `evals/evals.json` (64 cases) with a JSON schema at `evals/evals.schema.json` for IDE validation. Description trigger queries for optimizing the skill description are in `evals/train_queries.json` and `evals/validation_queries.json`. See `CLAUDE.md` for the full eval workflow: spawning runs, grading, blind comparison, aggregation, analysis, human review, and iteration.
